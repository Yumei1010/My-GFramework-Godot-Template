...
基于 [GFramework](https://github.com/GeWuYou/GFramework) (v0.7.1) 的 Godot 4.7 项目起手模板，**经个人重度实战项目魔改后反向提炼**，贴合个人使用习惯。
...
GFramework（上游 CQRS/ECS 框架）
    ↓
个人实战项目（重度魔改 GFramework 用法）
    ↓
My-GFramework-Godot-Template（剥离业务逻辑，保留骨架）
```

本模板并非 GFramework 官方模板，与上游 [GFramework-Godot-Template](https://github.com/GeWuYou/GFramework-Godot-Template) 出发点相似但走向不同。实战项目在大量开发中沉淀下来的**个人偏好**——DI 模块划分、partial class 拆分粒度、CQRS 事件/命令的 `sealed` + `init` 约束、XML 注释规范、Godot 节点注入模式等——都保留在了这套骨架里。
