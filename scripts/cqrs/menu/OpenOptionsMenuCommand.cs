...
public class OpenOptionsMenuCommand(OpenOptionsMenuCommandInput input) : AbstractCommand<OpenOptionsMenuCommandInput, UiHandle>(input)
{
    /// <summary>
    ///     执行打开设置菜单的核心逻辑。
    ///     通过获取 UI 路由系统实例，显示指定的设置菜单界面。
    /// </summary>
    /// <param name="input">包含执行所需数据的输入对象，其中可能包含一个可选的 UI 句柄。</param>
    /// <returns>返回一个 UiHandle 对象，表示当前显示或恢复的设置菜单界面句柄。</returns>
    protected override UiHandle OnExecute(OpenOptionsMenuCommandInput input)
    {
        // 获取 UI 路由系统实例，用于管理界面的显示与隐藏。
        var uiRouter = this.GetSystem<IUiRouter>()!;
        var handle = input.Handle;
        // 如果输入中未提供句柄，则直接显示设置菜单界面。
        if (!handle.HasValue) return uiRouter.Show(OptionsMenu.UiKeyStr, UiLayer.Modal);
        // 获取输入中的句柄值。
        var h = handle.Value;
        // 检查指定句柄是否已在模态层中存在，若不存在则显示设置菜单界面。
        if (uiRouter.GetFromLayer(h, UiLayer.Modal) is null) return uiRouter.Show(OptionsMenu.UiKeyStr, UiLayer.Modal);
